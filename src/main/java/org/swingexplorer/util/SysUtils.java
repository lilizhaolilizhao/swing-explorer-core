...
public abstract class SysUtils {
	 ...
	 * @param tree
	 */
	public static void collapseTree(JTree tree, boolean flag) {
	    TreeNode root = (TreeNode) tree.getModel().getRoot();
	    collapseTree(tree, new TreePath(root), flag);
	}

	/**
	 * @param tree
	 * @param parent
	 * @param flag=true时，关闭JTree到root; flag=false时，关闭JTree到第二层;
	 */
	private static void collapseTree(JTree tree, TreePath parent, boolean flag) {
	    TreeNode node = (TreeNode) parent.getLastPathComponent();
	    if (node.getChildCount() >= 0) {
	       for (Enumeration<?> e = node.children(); e.hasMoreElements();) {
	           TreeNode n = (TreeNode) e.nextElement();
	           TreePath path = parent.pathByAddingChild(n);
	           collapseTree(tree, path, flag);
	       }
	    }
	    if (flag) {
